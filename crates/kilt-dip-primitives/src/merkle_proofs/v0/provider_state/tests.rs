...
mod parachain_dip_did_proof {
	...
	use hex_literal::hex;
	use idchain_parachain_mainnet_runtime::Runtime as ParachainMainnetRuntime;
	use sp_core::H256;
	...
	fn verify_provider_head_proof_with_state_root_successful() {
		...
		let proof_verification_result = proof
			.verify_provider_head_proof_with_state_root::<BlakeTwo256, HeaderFor<ParachainMainnetRuntime>>(
				2_086,
				&relay_state_root,
			)
			.unwrap();
	...
	fn verify_provider_head_proof_with_state_root_wrong_relay_hasher() {
		...
		assert_err!(
			...
			proof.verify_provider_head_proof_with_state_root::<Keccak256, HeaderFor<ParachainMainnetRuntime>>(
				2_086,
				&relay_state_root
			),
	...
	fn verify_provider_head_proof_with_state_root_wrong_para_id() {
		...
		let proof = ParachainDipDidProof::<_, (), (), _, (), (), ()>::with_provider_head_proof(provider_head_proof);
		assert_err!(
			proof.verify_provider_head_proof_with_state_root::<BlakeTwo256, HeaderFor<ParachainMainnetRuntime>>(
				1_000,
				&relay_state_root
			),
			Error::ParaHeadMerkleProof(MerkleProofError::InvalidProof)
...
mod dip_did_proof_with_verified_relay_state_root {
	...
	// Storage proof generated at local Parachain Testnet instance (with new storage
	// hasher) for storage key
...
mod dip_did_proof_with_verified_subject_commitment {
	...
	// DIP proof generated on Parachain Testnet via the `dipProvider::generateProof`
	// runtime API.
