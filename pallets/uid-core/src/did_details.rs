...
/// verification keys a DID might control.
...
pub struct DidDetails<T: Config> {
	...
	/// \[OPTIONAL\] The ID of the verification key, used to verify the
	...
	/// verification, and delegation. Other than those, the map also contains
	/// the old verification keys that have been rotated, i.e., they cannot
...
impl<T: Config> DidDetails<T> {
	...
	/// Update the DID verification key, replacing the old one with the new one.
	///
	/// The old key is deleted from the set of public keys if it is
	/// not used in any other part of the DID. The new key is added to the
	/// set of public keys.
	...
	/// i.e., authentication, key agreement, verification, or delegation, is
	/// referencing it.
	...
	MaxNewKeyAgreementKeys: Get<u32> + Clone,
{
	...
	/// \[OPTIONAL\] The new verification key.
