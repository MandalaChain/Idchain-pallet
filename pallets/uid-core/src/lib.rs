...
//!   pallet](../../delegation/).
//!
//! - Zero or one **verification key**: used to sign and authorise the creation
//!   of new attested claims on the KILT blockchain. In case no verification key
//!   is present, the DID subject cannot write new attested claims on the KILT
//!   blockchain. For more info, check the [verification
//!   pallet](../../verification/).
//!
//! - A set of **public keys**: includes at least the previous keys in addition
//!   to any past verification key that has been rotated but not entirely
//!   revoked.
//!
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + Debug {
		...
		/// authentication, key agreement, verification, and delegation.
		#[pallet::constant]
		...
		T::AccountId: AsRef<[u8; 32]> + From<[u8; 32]>,
	{
		...
		/// Set or update the DID verification key.
		///
		/// If an old key existed, it is deleted from the set of public keys if
		/// it is not used in any other part of the DID. The new key is added to
		/// the set of public keys.
		...
		pub fn set_attestation_key(
			...
			log::debug!("Setting new verification key {:?} for DID {:?}", &new_key, &did_subject);
			did_details
		...
		/// Remove the DID verification key.
		///
		/// The old key is deleted from the set of public keys if
		/// it is not used in any other part of the DID.
		///
		...
		pub fn remove_attestation_key(origin: OriginFor<T>) -> DispatchResult {
			...
			log::debug!("Removing verification key for DID {:?}", &did_subject);
		...
		T::AccountId: AsRef<[u8; 32]> + From<[u8; 32]>,
	{
		...
		pub fn verify_did_operation_signature_and_increase_nonce(
			...
			let mut did_details = Did::<T>::get(&operation.did).ok_or(StorageError::NotFound(errors::NotFoundKind::Did))?;
