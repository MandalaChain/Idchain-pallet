...
///     * the CType of the delegation root matches the CType of the verification
///
...
///
/// Can remove verifications if (the same as revoke)
...
impl<T: Config>
	verification::VerificationAccessControl<DelegatorIdOf<T>, DelegationNodeIdOf<T>, CtypeHashOf<T>, ClaimHashOf<T>>
	for DelegationAc<T>
{
	fn can_attest(
		&self,
	...
	fn can_revoke(
		...
		// attester (provided by the verification pallet through on-chain storage) can be
		// different!
...
mod tests {
	...
	use verification::{mock::generate_base_verification, VerificationAccessControl};
	...
	fn test_can_attest() {
		...
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_eq!(stored_verification.attester, delegate);
				assert_eq!(
	...
	fn test_can_revoke_same_node() {
		...
		let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_01);
		let mut verification = generate_base_verification::<Test>(delegate.clone(), ACCOUNT_00);
		verification.authorization_id = Some(parent_id);

		ExtBuilder::default()
			.with_balances(vec![(ACCOUNT_00, <Test as Config>::Deposit::get() * 100)])
			.with_ctypes(vec![(verification.ctype_hash, delegate.clone())])
			.with_delegation_hierarchies(vec![(hierarchy_root_id, hierarchy_details, root_owner, ACCOUNT_00)])
			.with_delegations(vec![(parent_id, parent_node)])
	...
	fn test_can_revoke_parent() {
		...
		ExtBuilder::default()
			.with_balances(vec![(ACCOUNT_00, <Test as Config>::Deposit::get() * 100)])
			.with_ctypes(vec![(verification.ctype_hash, delegate)])
