...
pub mod pallet {
	...
	use verification::{Verifications, ClaimHashOf};
	use delegation::{DelegationNodeIdOf, DelegationNodes};
	use uid_core::{Did, DidIdentifierOf};
	...
	where
		...
		T: Config,
	{
		pub verification: BoundedVec<ClaimHashOf<T>, <T as Config>::MaxMigrationsPerPallet>,
	...
	where
		<T as verification::Config>::Currency: ReservableCurrency<
			AccountIdOf<T>,
			Balance = <<T as verification::Config>::Currency as Inspect<AccountIdOf<T>>>::Balance,
		>,
		...
		>,
		<T as uid_core::Config>::Currency: ReservableCurrency<
			AccountIdOf<T>,
			Balance = <<T as uid_core::Config>::Currency as Inspect<AccountIdOf<T>>>::Balance,
		>,
		...
		>,
	{
		...
		#[pallet::weight({
			...
			// TODO: Replace attestation with verification_migration_weight
			let mut weight = <T as crate::Config>::WeightInfo::attestation_migration_weight().saturating_mul(requested_migrations.verification.len().saturated_into());
		...
		pub fn update_balance(origin: OriginFor<T>, requested_migrations: EntriesToMigrate<T>) -> DispatchResult {
			...
			requested_migrations.verification.iter().try_for_each(|key| {
				let storage_key = Verifications::<T>::hashed_key_for(key);
				...
				if !is_migrated {
					...
					verification::migrations::update_balance_for_verification::<T>(key)
				} else {
					Ok(())
				}
			})?;
			...
			requested_migrations.did.iter().try_for_each(|key| {
				...
				if !is_migrated {
					let key_hash = <T as frame_system::Config>::Hashing::hash(&storage_key);
					MigratedKeys::<T>::insert(key_hash, ());
					uid_core::migrations::update_balance_for_did::<T>(key)
				} else {
					Ok(())
				}
			})?;
