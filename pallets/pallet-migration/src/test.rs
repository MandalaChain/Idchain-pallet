...
use verification::{
	mock::{claim_hash_from_seed, generate_base_verification, sr25519_did_from_public_key},
	Verifications, AttesterOf,
};
...
fn check_succesful_migration() {
	...
	verification.deposit.amount = MICRO_KILT;
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let attestaion_deposit =
				pallet_balances::Pallet::<Test>::balance_on_hold(&verification::HoldReason::Deposit.into(), &ACCOUNT_00);

			let did_deposit =
				pallet_balances::Pallet::<Test>::balance_on_hold(&uid_core::HoldReason::Deposit.into(), &ACCOUNT_00);
...
#[test]
fn check_excluded_keys_verification() {
	// attestaion
	let attester: AttesterOf<Test> = sr25519_did_from_public_key(&ALICE_SEED);
	let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_12);
	let mut verification = generate_base_verification::<Test>(attester.clone(), ACCOUNT_00);
	...
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let hashed_key = Verifications::<Test>::hashed_key_for(claim_hash);
...
fn check_excluded_keys_did() {
	...
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let hashed_key = uid_core::Did::<Test>::hashed_key_for(&alice_did);
			...
			assert_ok!(Did::create(
				RuntimeOrigin::signed(ACCOUNT_00),
				Box::new(details),
				uid_core::DidSignature::from(signature),
			));
...
fn migrate_key_by_update_deposit_did() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let did_key = uid_core::Did::<Test>::hashed_key_for(&alice_did);
...
#[test]
fn migrate_key_by_update_deposit_verification() {
	// verification
	let attester: AttesterOf<Test> = sr25519_did_from_public_key(&ALICE_SEED);
	let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_12);
