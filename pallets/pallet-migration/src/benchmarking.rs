...
use verification::{VerificationDetails, VerificationDetailsOf};
...
benchmarks! {
	where_clause {
		...
		<T as verification::Config>::Currency: ReservableCurrency<AccountIdOf<T>, Balance = <<T as verification::Config>::Currency as Inspect<AccountIdOf<T>>>::Balance>,
		...
		<T as uid_core::Config>::Currency: ReservableCurrency<AccountIdOf<T>, Balance = <<T as uid_core::Config>::Currency as Inspect<AccountIdOf<T>>>::Balance>,
		...
		<T as uid_core::Config>::DidIdentifier: From<AccountId32>,
		...
		T: ctype::Config<CtypeCreatorId = <T as verification::Config>::AttesterId>,
	...
	attestation_migration_weight {
		...
		let attester: <T as verification::Config>::AttesterId = account("attester", 0, SEED);
		...
		let details : VerificationDetailsOf<T> = VerificationDetails {
			attester,
		...
		verification::Verifications::<T>::insert(claim_hash, details.clone());
		...
		let entries_to_migrate = EntriesToMigrate {
			verification : BoundedVec::try_from(vec![claim_hash]).expect("Vector initialization should not fail."),
			..Default::default()
		};
	...
	did_migration_weight {
		...
		//create only one did. The migration for did, delegation, verification, w3n, public credentials, and did lookup is the same.
		...
		uid_core::Did::<T>::insert(did_subject.clone(), did_details.clone());
		pallet_balances::Pallet::<T>::set_balance(&sender, KILT.saturated_into());
