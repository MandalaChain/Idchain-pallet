...
#[cfg(test)]
pub mod runtime {
	use verification::mock::MockAccessControl;
	...
	type AttesterOf<T> = <T as verification::Config>::AttesterId;
	...
	parameter_types! {
		...
		// IMPORTANT: Needs to be at least MaxTotalKeyAgreementKeys + 3 (auth, delegation, verification keys) for benchmarks!
		#[derive(Debug, Clone)]
	...
	pub(crate) struct ExtBuilder {
		...
		verifications: Vec<(verification::ClaimHashOf<Test>, verification::VerificationDetailsOf<Test>)>,
		...
		dids_stored: Vec<(DidIdentifier, uid_core::did_details::DidDetails<Test>)>,
		connections: Vec<(
			AccountId,
			SubjectId,
	...
	impl ExtBuilder {
		...
		#[must_use]
		pub fn with_dids(mut self, dids: Vec<(DidIdentifier, uid_core::did_details::DidDetails<Test>)>) -> Self {
			self.dids_stored = dids;
			self
		}

		#[must_use]
		pub fn with_verifications(
			mut self,
			verifications: Vec<(verification::ClaimHashOf<Test>, verification::VerificationDetailsOf<Test>)>,
		) -> Self {
		...
		pub(crate) fn build(self) -> sp_io::TestExternalities {
			...
			ext.execute_with(|| {
				...
				for (claim_hash, details) in self.verifications {
					verification::mock::insert_verification::<Test>(claim_hash, details);
				}

				for did in self.dids_stored.iter() {
					uid_core::Did::<Test>::insert(&did.0, did.1.clone());
					pallet_balances::Pallet::<Test>::hold(
						&uid_core::HoldReason::Deposit.into(),
						&did.1.deposit.owner,
	...
	pub(crate) fn translate_all_holds_to_reserves() {
		let deposit_reasons: Vec<RuntimeHoldReason> = vec![
			verification::HoldReason::Deposit.into(),
