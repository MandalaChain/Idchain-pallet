...
use crate::{self as verification, mock::*, AttesterOf, Config, Error, HoldReason};
...
fn test_reclaim_deposit_not_found() {
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			assert_noop!(
				Verification::reclaim_deposit(RuntimeOrigin::signed(ACCOUNT_01), claim_hash),
...
fn test_change_deposit_owner() {
	...
	verification.authorization_id = Some(other_authorized);
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_ok!(Verification::change_deposit_owner(
				DoubleOrigin(ACCOUNT_01, attester).into(),
				claim_hash
			));
			assert_eq!(
				Verification::verifications(claim_hash)
					.expect("Verification must be retained")
					.deposit,
				Deposit {
					owner: ACCOUNT_01,
...
fn test_change_deposit_owner_insufficient_balance() {
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_noop!(
				Verification::change_deposit_owner(DoubleOrigin(ACCOUNT_01, attester).into(), claim_hash),
...
fn test_change_deposit_owner_unauthorized() {
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			assert_noop!(
				Verification::change_deposit_owner(DoubleOrigin(ACCOUNT_00, evil_actor).into(), claim_hash),
...
fn test_change_deposit_owner_not_found() {
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			assert_noop!(
				Verification::change_deposit_owner(DoubleOrigin(ACCOUNT_00, attester).into(), claim_hash),
				verification::Error::<Test>::NotFound,
			);
		});
}
...
fn test_update_deposit() {
	let attester: AttesterOf<Test> = sr25519_did_from_public_key(&BOB_SEED);
	let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_01);
	let verification = generate_base_verification_with_deposit::<Test>(
		attester.clone(),
		ACCOUNT_00,
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_ok!(Verification::update_deposit(
...
fn test_update_deposit_unauthorized() {
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_noop!(
				Verification::update_deposit(RuntimeOrigin::signed(ACCOUNT_01), claim_hash),
...
fn test_reclaim_deposit() {
	...
	let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_01);
	let mut verification = generate_base_verification::<Test>(attester.clone(), ACCOUNT_00);
	verification.authorization_id = Some(other_authorized.clone());
	...
	ExtBuilder::default()
		...
		.build_and_execute_with_sanity_tests(|| {
			...
			assert_ok!(Verification::reclaim_deposit(
				RuntimeOrigin::signed(ACCOUNT_00),
				claim_hash
			));
			assert!(!Verification::external_verifications(
