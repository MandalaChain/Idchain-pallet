...
benchmarks! {
	...
	verify {
		assert!(Verifications::<T>::contains_key(claim_hash));
		assert_eq!(Pallet::<T>::verifications(claim_hash), Some(VerificationDetails {
			ctype_hash,
			attester,
	...
	verify {
		assert!(Verifications::<T>::contains_key(claim_hash));
		assert_eq!(Verifications::<T>::get(claim_hash), Some(VerificationDetails {
			ctype_hash,
			attester,
			authorization_id: None,
			revoked: true,
