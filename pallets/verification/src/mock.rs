...
//! other tests. Internal functions/structs can only be used in verification
//! tests.
...
use crate::{
	pallet::AuthorizationIdOf, AccountIdOf, VerificationAccessControl, VerificationDetails, VerificationDetailsOf,
...
pub struct VerificationCreationDetails<T: Config> {
	pub claim_hash: ClaimHashOf<T>,
	pub ctype_hash: CtypeHashOf<T>,
	pub authorization_id: Option<AuthorizationIdOf<T>>,
}

pub fn generate_base_verification_creation_details<T: Config>(
	claim_hash: ClaimHashOf<T>,
	verification: VerificationDetailsOf<T>,
) -> VerificationCreationDetails<T> {
	VerificationCreationDetails {
		claim_hash,
		ctype_hash: verification.ctype_hash,
		authorization_id: verification.authorization_id,
	}
}

pub fn generate_base_verification<T>(attester: AttesterOf<T>, payer: AccountIdOf<T>) -> VerificationDetailsOf<T>
where
	T: Config,
	T::Hash: From<H256>,
{
	generate_base_verification_with_deposit::<T>(attester, payer, <T as Config>::Deposit::get())
}

pub fn generate_base_verification_with_deposit<T>(
	attester: AttesterOf<T>,
	payer: AccountIdOf<T>,
	deposit: BalanceOf<T>,
) -> VerificationDetailsOf<T>
where
	T: Config,
	T::Hash: From<H256>,
{
...
impl<T> VerificationAccessControl<T::AttesterId, T::AuthorizationId, CtypeHashOf<T>, ClaimHashOf<T>>
...
pub fn insert_verification<T: Config>(claim_hash: ClaimHashOf<T>, details: VerificationDetailsOf<T>) {
	crate::VerificationStorageDepositCollector::<T>::create_deposit(
		details.deposit.owner.clone(),
		details.deposit.amount,
	)
	.expect("Should have balance");

	crate::Verifications::<T>::insert(claim_hash, details.clone());
	if let Some(delegation_id) = details.authorization_id.as_ref() {
		crate::ExternalVerifications::<T>::insert(delegation_id, claim_hash, true)
	}
}
...
pub(crate) mod runtime {
	...
	use crate::{self as verification};
	...
	pub const VERIFICATION_DEPOSIT: Balance = 10 * MILLI_UNIT;
	...
	parameter_types! {
		...
		pub const VerificationDeposit: Balance = VERIFICATION_DEPOSIT;
	}
	...
	pub struct ExtBuilder {
		...
		verifications: Vec<(ClaimHashOf<Test>, VerificationDetailsOf<Test>)>,
	}
	...
	impl ExtBuilder {
		...
		#[must_use]
		pub fn with_verifications(mut self, verifications: Vec<(ClaimHashOf<Test>, VerificationDetailsOf<Test>)>) -> Self {
		...
		pub fn build(self) -> sp_io::TestExternalities {
			...
			ext.execute_with(|| {
				...
				for (claim_hash, details) in self.verifications {
					insert_verification::<Test>(claim_hash, details);
				}
			});
		...
		pub fn build_and_execute_with_sanity_tests(self, test: impl FnOnce()) {
			self.build().execute_with(|| {
				test();
				crate::try_state::do_try_state::<Test>().expect("Sanity test for verification failed.");
			})
		}
