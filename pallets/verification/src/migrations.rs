...
use crate::{AccountIdOf, Verifications, ClaimHashOf, Config, CurrencyOf, Error, HoldReason};

pub fn update_balance_for_verification<T: Config>(key: &ClaimHashOf<T>) -> DispatchResult
where
	<T as Config>::Currency:
		ReservableCurrency<T::AccountId, Balance = <<T as Config>::Currency as Inspect<AccountIdOf<T>>>::Balance>,
{
	let details = Verifications::<T>::get(key).ok_or(Error::<T>::NotFound)?;
...
pub mod test {
	...
	use crate::{
		migrations::update_balance_for_verification, mock::*, AccountIdOf, Verifications, AttesterOf, Config, Error,
		HoldReason,
	};
	...
	fn test_setup() {
		...
		let verifications = generate_base_verification::<Test>(attester.clone(), ACCOUNT_00);

		ExtBuilder::default()
			.with_ctypes(vec![(ctype_hash, attester)])
			.with_balances(vec![(ACCOUNT_00, <Test as Config>::Deposit::get() * 100)])
			.with_verifications(vec![(claim_hash, verifications)])
	...
	#[test]
	fn test_balance_migration_verification() {
		let attester: AttesterOf<Test> = sr25519_did_from_public_key(&ALICE_SEED);
		let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_01);
		...
		ExtBuilder::default()
			...
			.build_and_execute_with_sanity_tests(|| {
				...
				let verification_pre_migration = Verifications::<Test>::get(claim_hash);
				...
				//verifications should be in storage
				assert!(verification_pre_migration.is_some());

				// before the migration the deposit should be reserved.
				assert_eq!(
					reserved_pre_migration,
					verification_pre_migration.clone().unwrap().deposit.amount
				);

				assert!(update_balance_for_verification::<Test>(&claim_hash).is_ok());

				let verification_post_migration = Verifications::<Test>::get(claim_hash);
				...
				//verifications should be still in the storage
				assert!(verification_post_migration.is_some());

				// ... and should be the same
				assert_eq!(verification_post_migration, verification_pre_migration);
				...
				assert_noop!(
					update_balance_for_verification::<Test>(&claim_hash2),
					Error::<Test>::NotFound
				);
			});
	}
