...
/// Native IDCHAIN parachain-mainnet executor instance.
pub(crate) struct IdChainParachainMainnetRuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for IdChainParachainMainnetRuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		idchain_parachain_mainnet_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		idchain_parachain_mainnet_runtime::native_version()
	}
}

/// Native IDCHAIN parachain-testnet executor instance.
pub(crate) struct IdChainParachainTestnetRuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for IdChainParachainTestnetRuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		idchain_parachain_testnet_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		idchain_parachain_testnet_runtime::native_version()
	}
}
	...
	) -> Result<sc_consensus::DefaultImportQueue<Block>, sc_service::Error>,
{
	...
	.await
	.map_err(|e| sc_service::Error::Application(Box::new(e).into()))?;
