...
# Additional check for expected directories
if [ ! -d "zombienet" ]; then
    echo "Error: The 'zombienet' directory is missing. Are you sure you're in the project root?"
    print_usage
    exit 1
fi
...
# Copy the binary instead of moving it
sudo cp "$BINARIES_DIR/zombienet" "$COMMON_DIR/zombienet"

# Make sure it's executable for all users
sudo chmod a+x "$COMMON_DIR/zombienet"

# Create a symlink in the original location
ln -sf "$COMMON_DIR/zombienet" "$BINARIES_DIR/zombienet"

echo "Zombienet binary copied to $COMMON_DIR and made globally executable."
echo "A symlink has been created in $BINARIES_DIR for compatibility."

# Verify the installation
if command -v zombienet &> /dev/null; then
    echo "Zombienet is now available globally. You can run it using: zombienet"
else
    echo "Zombienet installation might have failed. Please check your system PATH."
fi

# Inform about the local copy
echo "A local copy of Zombienet is also available at: $BINARIES_DIR/zombienet"
