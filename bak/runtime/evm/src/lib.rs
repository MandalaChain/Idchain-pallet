...
use pallet_ethereum::{TransactionAction, TransactionData};
...
mod precompiles;
use precompiles::IdChainPrecompiles;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("ID chain"),
    impl_name: create_runtime_str!("ID chain"),
    authoring_version: 1,
    spec_version: 5,
    impl_version: 1,
...
parameter_types! {
    ...
    pub PrecompilesValue: IdChainPrecompiles<Runtime> = IdChainPrecompiles::<_>::new();
...
impl pallet_evm::Config for Runtime {
    ...
    type RuntimeEvent = RuntimeEvent;
    type PrecompilesType = IdChainPrecompiles<Self>;
...
pub const MICROBGLT: Balance = 1_000_000_000_000;
pub const MILLIBGLT: Balance = 1_000 * MICROBGLT;
pub const BGLT: Balance = 1_000 * MILLIBGLT;

/// Charge fee for stored bytes and items.
pub const fn deposit(items: u32, bytes: u32) -> Balance {
    (items as Balance) * 1 * BGLT + (bytes as Balance) * STORAGE_BYTE_FEE
}

pub const STORAGE_BYTE_FEE: Balance = MICROBGLT / 1000;
...
impl pallet_contracts::Config for Runtime {
    ...
    // just 1 byte less than the max code bytes
    type MaxCodeLen = ConstU32<58876>;
    ...
    type MaxDelegateDependencies = ConstU32<0>;
}
