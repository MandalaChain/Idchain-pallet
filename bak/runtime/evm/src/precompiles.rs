...
pub struct IdChainPrecompiles<R>(PhantomData<R>);

impl<R> IdChainPrecompiles<R>
where
    R: pallet_evm::Config,
{
    pub fn new() -> Self {
    ...
    pub fn used_addresses() -> [H160; 9] {
        [
            ...
            hash(1025),
            hash(1026),
            // id chain specific precompiles (2000-3000)
            hash(2000),
        ]
    }
}

impl<Runtime> PrecompileSet for IdChainPrecompiles<Runtime>
where
    Runtime: pallet_evm::Config + pallet_timestamp::Config,
{
    fn execute(&self, handle: &mut impl PrecompileHandle) -> Option<PrecompileResult> {
        match handle.code_address() {
            ...
            // id chain specific precompiles (2000-3000)
            // CallPermitPrecompile at address : 0x00000000000000000000000000000000000007d0
            address if address == hash(2000) => {
                Some(CallPermitPrecompile::<Runtime>::execute(handle))
            }
