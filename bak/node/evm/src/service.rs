...
use sp_core::U256;
// Runtime
use evm_runtime::{opaque::Block, Hash, TransactionConverter};
...
// Creates a new partial client and import queue for the node.
///
/// # Parameters
///
/// - `config`: The configuration for the service.
/// - `eth_config`: The Ethereum-specific configuration for the service.
/// - `build_import_queue`: A closure to build the import queue.
///
/// # Returns
///
/// A `Result` containing a `PartialComponents` struct with the components needed for the full node,
/// or a `ServiceError` if the components could not be created.
///
/// The `PartialComponents` struct contains:
/// - `client`: The full client instance.
/// - `backend`: The full backend instance.
/// - `keystore_container`: The keystore container.
/// - `task_manager`: The task manager.
/// - `select_chain`: The select chain instance.
/// - `import_queue`: The import queue.
/// - `transaction_pool`: The transaction pool.
/// - `other`: A tuple containing:
///     - `telemetry`: An optional telemetry instance.
///     - `block_import`: The block import instance.
///     - `grandpa_link`: The Grandpa link half.
///     - `frontier_backend`: The Frontier backend instance.
///     - `overrides`: The overrides handle.
    ...
    Executor: NativeExecutionDispatch + 'static,
{
    ...
    impl sp_inherents::InherentDataProvider for MockTimestampInherentDataProvider {
        async fn provide_inherent_data(
            ...
            TIMESTAMP.with(|x| {
                *x.borrow_mut() += evm_runtime::SLOT_DURATION;
...
/// Builds the full node, setting up all necessary components and configurations.
///
/// # Parameters
///
/// - `config`: The configuration for the service.
/// - `eth_config`: The Ethereum-specific configuration for the service.
/// - `sealing`: Optional sealing configuration.
///
/// # Returns
///
/// A `Result` containing the `TaskManager` instance for the full node, or a `ServiceError` if the node could not be built.
///
/// This function asynchronously calls `new_full` with the provided runtime API and executor,
/// and returns the task manager for managing the tasks of the node.
pub async fn build_full(
    config: Configuration,
    eth_config: EthConfiguration,
    sealing: Option<Sealing>,
) -> Result<TaskManager, ServiceError> {
    new_full::<evm_runtime::RuntimeApi, RuntimeExecutor>(config, eth_config, sealing).await
}

/// Creates a new chain operations handler, setting up the necessary components and configurations.
///
/// # Parameters
///
/// - `config`: A mutable reference to the configuration for the service.
/// - `eth_config`: The Ethereum-specific configuration for the service.
///
/// # Returns
///
/// A `Result` containing a tuple with the following components if successful, or a `ServiceError` if the components could not be created:
/// - `Arc<Client>`: The full client instance.
/// - `Arc<FullBackend>`: The full backend instance.
/// - `BasicQueue<Block>`: The basic import queue.
/// - `TaskManager`: The task manager.
/// - `FrontierBackend`: The Frontier backend instance.
///
/// This function sets the keystore configuration to in-memory, then creates the partial components
/// using the `new_partial` function. It extracts the necessary components from the `PartialComponents`
/// struct and returns them as a tuple.
...
> {
    ...
    let PartialComponents {
        ...
        other,
        ..
    } = new_partial::<evm_runtime::RuntimeApi, RuntimeExecutor, _>(
        config,
        eth_config,
