//! Main module for running the ID CHAIN node CLI.
...
use crate::{
    chain_spec::{self, CustomChainSpecProperties, Dev, Local},
    cli::{Cli, Subcommand},
...
impl SubstrateCli for Cli {
    ...
    /// Returns the author(s) of the implementation.
    fn author() -> String {
    ...
    /// Returns the starting year of the copyright.
    fn copyright_start_year() -> i32 {
        2024
    }

    /// Loads the chain specification based on the provided identifier.
    fn load_spec(&self, id: &str) -> Result<Box<dyn ChainSpec>, String> {
        Ok(match id {
            "local" => Box::new(chain_spec::NodeChainSpec::<Local>::build()),
            "dev" => Box::new(chain_spec::NodeChainSpec::<Dev>::build()),
...
/// Parses and runs command line arguments.
pub fn run() -> sc_cli::Result<()> {
    ...
    match &cli.subcommand {
        ...
        Some(Subcommand::ChainInfo(cmd)) => {
            let runner = cli.create_runner(cmd)?;
            runner.sync_run(|config| cmd.run::<evm_runtime::Block>(&config))
        }
        None => {
