...
use xcm::{latest::prelude::BodyId, v4::Location, VersionedAssetId};
...
use runtime_common::{
	...
	constants::{
		self, UnvestedFundsAllowedWithdrawReasons, BLOCK_PROCESSING_VELOCITY, EXISTENTIAL_DEPOSIT, HOURS, KILT,
	...
	pallet_id,
	xcm_config::{RelayLocation, RelayOrigin},
...
construct_runtime! {
	pub enum Runtime
	{
		...
		// DELETED: ParachainStaking: parachain_staking = 21,
		...
		Ctype: uid_credential = 61,
		Attestation: verification = 62,
		...
		// Fix: delete this pallet in the next release
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("idchain-parachain-mainnet"),
	impl_name: create_runtime_str!("idchain-parachain-mainnet"),
	authoring_version: 1,
	spec_version: 11401,
	impl_version: 0,
...
parameter_types! {
	pub const Period: u32 = (6 * HOURS) as u32;
...
impl pallet_session::Config for Runtime {
	...
	// We don't have stash and controller, thus no need for convert as well.
...
impl verification::Config for Runtime {
	type EnsureOrigin = uid_core::EnsureDidOrigin<DidIdentifier, AccountId>;
	type OriginSuccess = uid_core::DidRawOrigin<AccountId, DidIdentifier>;
	type RuntimeHoldReason = RuntimeHoldReason;
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = weights::verification::WeightInfo<Runtime>;

	type Currency = Balances;
	type Deposit = constants::verification::VerificationDeposit;
	type MaxDelegatedVerifications = constants::verification::MaxDelegatedVerifications;
...
impl delegation::Config for Runtime {
	...
	#[cfg(not(feature = "runtime-benchmarks"))]
	type DelegationSignatureVerification = uid_core::DidSignatureVerify<Runtime>;
	#[cfg(not(feature = "runtime-benchmarks"))]
	type Signature = uid_core::DidSignature;
...
impl uid_credential::Config for Runtime {
...
impl uid_core::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type RuntimeCall = RuntimeCall;
	type RuntimeHoldReason = RuntimeHoldReason;
	type RuntimeOrigin = RuntimeOrigin;
...
impl pallet_web3_names::Config for Runtime {
	...
	type OwnerOrigin = uid_core::EnsureDidOrigin<DidIdentifier, AccountId>;
	type OriginSuccess = uid_core::DidRawOrigin<AccountId, DidIdentifier>;
	type Currency = Balances;
		...
		match self {
			...
			ProxyType::NonDepositClaiming => matches!(
				...
				RuntimeCall::Attestation(
						...
						verification::Call::add { .. }
							| verification::Call::remove { .. }
							| verification::Call::revoke { .. }
							| verification::Call::change_deposit_owner { .. }
							| verification::Call::update_deposit { .. }
					)
					// Excludes `Balances`
					| RuntimeCall::Council(..)
					| RuntimeCall::Ctype(..)
					...
					| RuntimeCall::Did(
						// Excludes `reclaim_deposit`
						uid_core::Call::add_key_agreement_key { .. }
							| uid_core::Call::add_service_endpoint { .. }
							| uid_core::Call::create { .. }
							| uid_core::Call::delete { .. }
							| uid_core::Call::remove_attestation_key { .. }
							| uid_core::Call::remove_delegation_key { .. }
							| uid_core::Call::remove_key_agreement_key { .. }
							| uid_core::Call::remove_service_endpoint { .. }
							| uid_core::Call::set_attestation_key { .. }
							| uid_core::Call::set_authentication_key { .. }
							| uid_core::Call::set_delegation_key { .. }
							| uid_core::Call::submit_did_call { .. }
							| uid_core::Call::update_deposit { .. }
							| uid_core::Call::change_deposit_owner { .. }
							| uid_core::Call::create_from_account { .. }
							| uid_core::Call::dispatch_as { .. }
					)
					| RuntimeCall::DidLookup(
						// Excludes `reclaim_deposit`
...
impl uid_core::DeriveDidCallAuthorizationVerificationKeyRelationship for RuntimeCall {
	fn derive_verification_key_relationship(&self) -> uid_core::DeriveDidCallKeyRelationshipResult {
		/// ensure that all calls have the same VerificationKeyRelationship
		fn single_key_relationship(calls: &[RuntimeCall]) -> uid_core::DeriveDidCallKeyRelationshipResult {
			let init = calls
				.get(0)
				.ok_or(uid_core::RelationshipDeriveError::InvalidCallParameter)?
			...
			calls
				...
				.try_fold(init, |acc, next| {
					if next.is_err() {
						...
						Err(uid_core::RelationshipDeriveError::InvalidCallParameter)
					}
				})
		}
		match self {
			RuntimeCall::Attestation { .. } => Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod),
			RuntimeCall::Ctype { .. } => Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod),
			RuntimeCall::Delegation { .. } => Ok(uid_core::DidVerificationKeyRelationship::CapabilityDelegation),
			RuntimeCall::DipProvider { .. } => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
			// DID creation is not allowed through the DID proxy.
			RuntimeCall::Did(uid_core::Call::create { .. }) => Err(uid_core::RelationshipDeriveError::NotCallableByDid),
			RuntimeCall::Did { .. } => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
			RuntimeCall::Web3Names { .. } => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
			RuntimeCall::PublicCredentials { .. } => Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod),
			RuntimeCall::DidLookup { .. } => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
			...
			#[cfg(not(feature = "runtime-benchmarks"))]
			_ => Err(uid_core::RelationshipDeriveError::NotCallableByDid),
			// By default, returns the authentication key
			#[cfg(feature = "runtime-benchmarks")]
			_ => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
		}
	}
...
impl_runtime_apis! {
	...
	> for Runtime {
		...
		> {
			...
			pallet_web3_names::Owner::<Runtime>::get(&name)
				.and_then(|owner_info| {
					uid_core::Did::<Runtime>::get(&owner_info.owner).map(|details| (owner_info, details))
				})
				.map(|(owner_info, details)| {
					let accounts = pallet_did_lookup::ConnectedAccounts::<Runtime>::iter_key_prefix(
						&owner_info.owner,
					).collect();
					let service_endpoints = uid_core::ServiceEndpoints::<Runtime>::iter_prefix(&owner_info.owner).map(|e| From::from(e.1)).collect();
		...
		> {
			pallet_did_lookup::ConnectedDids::<Runtime>::get(account)
				.and_then(|owner_info| {
					uid_core::Did::<Runtime>::get(&owner_info.did).map(|details| (owner_info, details))
				})
				.map(|(connection_record, details)| {
					...
					let service_endpoints = uid_core::ServiceEndpoints::<Runtime>::iter_prefix(&connection_record.did).map(|e| From::from(e.1)).collect();
		...
		fn query(did: DidIdentifier) -> Option<
			...
			>
		> {
			let details = uid_core::Did::<Runtime>::get(&did)?;
			...
			let service_endpoints = uid_core::ServiceEndpoints::<Runtime>::iter_prefix(&did).map(|e| From::from(e.1)).collect();
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		fn on_runtime_upgrade(checks: UpgradeCheckSelect) -> (Weight, Weight) {
			log::info!("try-runtime::on_runtime_upgrade parachain-mainnet.");
		...
		fn execute_block(block: Block, state_root_check: bool, sig_check: bool, select: frame_try_runtime::TryStateSelect) -> Weight {
			log::info!(
				target: "runtime::parachain-mainnet", "try-runtime: executing block #{} ({:?}) / root checks: {:?} / sig check: {:?} / sanity-checks: {:?}",
				block.header.number,
				block.header.hash(),
				state_root_check,
				sig_check,
