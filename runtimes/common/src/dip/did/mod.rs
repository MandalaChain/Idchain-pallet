...
use uid_core::did_details::DidDetails;
...
where
	Runtime: uid_core::Config + pallet_web3_names::Config,
{
...
where
	Runtime: uid_core::Config<DidIdentifier = <Runtime as pallet_dip_provider::Config>::Identifier>
		...
		+ pallet_dip_provider::Config,
{
	...
	fn retrieve(identifier: &Runtime::Identifier) -> Result<Self::Success, Self::Error> {
			...
			uid_core::Pallet::<Runtime>::get_deleted_did(identifier).is_none(),
			LinkedDidInfoProviderError::DidDeleted,
		);
		let did_details = uid_core::Pallet::<Runtime>::get_did(identifier).ok_or(LinkedDidInfoProviderError::DidNotFound)?;
	...
	<Runtime as frame_system::Config>::AccountId: AsRef<[u8; 32]> + From<[u8; 32]>,
{
	...
	fn worst_case(context: IdentityContext<Runtime::Identifier, Runtime::AccountId>) -> Self::Output {
		...
		let max_new_keys = <Runtime as uid_core::Config>::MaxNewKeyAgreementKeys::get();
		...
		let did_details = uid_core::did_details::DidDetails::new_with_creation_details(
		...
		// add to storage.
		uid_core::Pallet::<Runtime>::try_insert_did(did.clone(), did_details.clone(), submitter.clone())
