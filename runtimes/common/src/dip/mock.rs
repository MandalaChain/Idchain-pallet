...
impl uid_core::Config for TestRuntime {
...
pub(crate) fn create_linked_info(
	...
	let did_details = {
		...
		details
			...
			.expect("Should not fail to add verification key to DID.");
		details
		...
		ext.execute_with(|| {
			...
			for did_identifier in self.1 {
				...
				let _ = Did::try_insert_did(
					...
					uid_core::mock_utils::generate_base_did_details(DidVerificationKey::Account(ACCOUNT), Some(SUBMITTER)),
					SUBMITTER,
				);
				uid_core::Pallet::<TestRuntime>::delete_did(did_identifier, 0)
