...
use uid_core::{did_details::DidDetails, DidVerificationKeyRelationship, KeyIdOf};
	...
	Runtime: uid_core::Config,
{
	...
	let att_key_details = did_details.public_keys.get(&att_key_id).ok_or_else(|| {
		log::error!(
			target: LOG_TARGET,
			"Failed to find verification  key {:#?} among the public keys {:#?}.",
			att_key_id,
			did_details.public_keys
		);
		DidMerkleProofError::Internal
...
where
	Runtime: uid_core::Config + pallet_did_lookup::Config + pallet_web3_names::Config,
{
	let LinkedDidInfoOf {
		did_details,
		web3_name_details,
...
where
	Runtime: uid_core::Config + pallet_did_lookup::Config + pallet_web3_names::Config + pallet_dip_provider::Config,
