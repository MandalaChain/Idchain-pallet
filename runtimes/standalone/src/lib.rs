...
construct_runtime!(
	pub enum Runtime
	{
		...
		Ctype: uid_credential = 9,
		Attestation: verification = 10,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("idchain-standalone-testnet"),
	impl_name: create_runtime_str!("idchain-standalone-testnet"),
	authoring_version: 4,
	spec_version: 11401,
	impl_version: 0,
...
parameter_types! {
	pub const MaxDelegatedVerifications: u32 = 1000;
	pub const VerificationDeposit: Balance = constants::verification::VERIFICATION_DEPOSIT;
}

impl verification::Config for Runtime {
	type RuntimeHoldReason = RuntimeHoldReason;
	type EnsureOrigin = uid_core::EnsureDidOrigin<DidIdentifier, AccountId>;
	type OriginSuccess = uid_core::DidRawOrigin<DidIdentifier, AccountId>;
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = ();
	type Currency = Balances;
	type Deposit = VerificationDeposit;
	type MaxDelegatedVerifications = MaxDelegatedVerifications;
...
impl delegation::Config for Runtime {
	...
	#[cfg(not(feature = "runtime-benchmarks"))]
	type DelegationSignatureVerification = uid_core::DidSignatureVerify<Self>;
...
impl uid_core::Config for Runtime {
	...
	#[cfg(not(feature = "runtime-benchmarks"))]
	type EnsureOrigin = uid_core::EnsureDidOrigin<Self::DidIdentifier, AccountId>;
	#[cfg(not(feature = "runtime-benchmarks"))]
	type OriginSuccess = uid_core::DidRawOrigin<AccountId, Self::DidIdentifier>;
		...
		fn single_key_relationship(calls: &[RuntimeCall]) -> uid_core::DeriveDidCallKeyRelationshipResult {
			...
			calls
				...
				.try_fold(init, |acc, next| {
					if Ok(acc) == next { Ok(acc) } else { Err(uid_core::RelationshipDeriveError::InvalidCallParameter) }
				})
		}
		match self {
...
impl_runtime_apis! {
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		fn on_runtime_upgrade(checks: UpgradeCheckSelect) -> (Weight, Weight) {
			log::info!("try-runtime::on_runtime_upgrade idchain-standalone runtime.");
