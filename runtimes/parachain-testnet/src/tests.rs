...
use uid_core::DeriveDidCallAuthorizationVerificationKeyRelationship;
...
use runtime_common::{
	constants::{
		verification::MAX_VERIFICATION_BYTE_LENGTH,
...
#[test]
fn verification_storage_sizes() {
	type DelegationRecord =
		BoundedVec<<Runtime as frame_system::Config>::Hash, <Runtime as verification::Config>::MaxDelegatedVerifications>;

	let verification_record = verification::VerificationDetailsOf::<Runtime>::max_encoded_len();
	let delegation_record = DelegationRecord::max_encoded_len()
		/ (<Runtime as verification::Config>::MaxDelegatedVerifications::get() as usize);
	assert_eq!(
		verification_record + delegation_record,
		MAX_VERIFICATION_BYTE_LENGTH as usize
	)
}
...
fn did_storage_sizes() {
	// Service endpoint
	let max_did_endpoint_size = uid_core::service_endpoints::DidEndpoint::<Runtime>::max_encoded_len();
	assert_eq!(max_did_endpoint_size, MAX_SERVICE_ENDPOINT_BYTE_LENGTH as usize);

	// DID key
	let max_did_key_size = uid_core::did_details::DidPublicKey::<AccountId>::max_encoded_len();
...
fn test_derive_did_verification_relation_ctype() {
	let c1 = RuntimeCall::UidCredential(uid_credential::Call::add {
		ctype: vec![0, 1, 2, 3],
	});
	let c2 = RuntimeCall::UidCredential(uid_credential::Call::add {
		ctype: vec![0, 1, 2, 3, 3],
	});
	let c3 = RuntimeCall::UidCredential(uid_credential::Call::add {
		ctype: vec![0, 1, 2, 3, 3],
	});
	let c4 = RuntimeCall::UidCredential(uid_credential::Call::add {
		ctype: vec![0, 1, 2, 100],
	});
	...
	assert_eq!(
		...
		Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod)
	);
}
...
fn test_derive_did_key_web3name() {
	assert_eq!(
		...
		Ok(uid_core::DidVerificationKeyRelationship::Authentication)
	);
...
fn test_derive_did_verification_relation_fail() {
	...
	assert_eq!(
		cb.derive_verification_key_relationship(),
		Err(uid_core::RelationshipDeriveError::NotCallableByDid)
	);
}
