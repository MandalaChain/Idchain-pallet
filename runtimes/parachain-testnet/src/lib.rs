...
construct_runtime! {
	pub enum Runtime
	{
		...
		Credential: uid_credential = 61,
		Verification: verification = 62,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("idchain-node"),
	impl_name: create_runtime_str!("idchain-node"),
	authoring_version: 4,
	spec_version: 11401,
	impl_version: 0,
	...
	fn filter(&self, c: &RuntimeCall) -> bool {
		match self {
			...
			ProxyType::NonTransfer => matches!(
				c,
				RuntimeCall::Verification(..)
					// Excludes `Balances`
					| RuntimeCall::Council(..)
					| RuntimeCall::Credential(..)
...
impl uid_core::DeriveDidCallAuthorizationVerificationKeyRelationship for RuntimeCall {
	fn derive_verification_key_relationship(&self) -> uid_core::DeriveDidCallKeyRelationshipResult {
		...
		match self {
			RuntimeCall::Verification { .. } => Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod),
			RuntimeCall::Credential { .. } => Ok(uid_core::DidVerificationKeyRelationship::AssertionMethod),
			...
			RuntimeCall::Uid(uid_core::Call::create { .. }) => Err(uid_core::RelationshipDeriveError::NotCallableByDid),
			RuntimeCall::Uid { .. } => Ok(uid_core::DidVerificationKeyRelationship::Authentication),
...
impl_runtime_apis! {
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		fn on_runtime_upgrade(checks: UpgradeCheckSelect) -> (Weight, Weight) {
			log::info!("try-runtime::on_runtime_upgrade parachain-testnet.");
		...
		fn execute_block(block: Block, state_root_check: bool, sig_check: bool, select: frame_try_runtime::TryStateSelect) -> Weight {
			log::info!(
				target: "runtime::parachain-testnet", "try-runtime: executing block #{} ({:?}) / root checks: {:?} / sig check: {:?} / sanity-checks: {:?}",
				block.header.number,
				block.header.hash(),
				state_root_check,
				sig_check,
