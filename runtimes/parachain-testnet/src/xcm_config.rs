...
parameter_types! {
	...
	// TODO: This needs to be updated once we deploy parachain-testnet on Rococo/Paseo
...
/// `AccountId` within the parachain-testnet network, which is crucial for determining
...
pub type XcmOriginToTransactDispatchOrigin = (
	...
	// In contrast to parachain-mainnet, it's fine to include this on parachain-testnet for testing.
...
impl Contains<RuntimeCall> for SafeCallFilter {
	fn contains(c: &RuntimeCall) -> bool {
		...
		match c {
			RuntimeCall::Uid(c) => match c {
				uid_core::Call::dispatch_as { call, .. } => is_call_allowed(call),
				uid_core::Call::submit_did_call { did_call, .. } => is_call_allowed(&did_call.call),
